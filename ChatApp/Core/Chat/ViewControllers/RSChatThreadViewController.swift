//
//  RSChatThreadViewController.swift
//  ChatApp
//
//  Created by Richard Stewart on 8/26/20.
//
...
class RSChatThreadViewController: MessagesViewController, MessagesDataSource, InputBarAccessoryViewDelegate {
  var user: RSUser
  private var messages: [RSChatMessage] = []
  ...
  var channel: RSChatChannel
  var uiConfig: RSChatUIConfiguration
  let remoteData = RSRemoteData()
  
  init(user: RSUser, channel: RSChatChannel, uiConfig: RSChatUIConfiguration) {
    self.user = user
  ...
  private func save(_ message: RSChatMessage) {
  ...
  private func insertNewMessage(_ message: RSChatMessage) {
  ...
  private func handleDocumentChange(_ change: DocumentChange) {
    guard var message = RSChatMessage(document: change.document) else {
      return
    }
  ...
  private func uploadImage(_ image: UIImage, to channel: RSChatChannel, completion: @escaping (URL?) -> Void) {
  ...
  private func sendPhoto(_ image: UIImage) {
    ...
    uploadImage(image, to: channel) { [weak self] url in
      ...
      var message = RSChatMessage(user: self.user, image: image)
  ...
  func messageForItem(at indexPath: IndexPath,
    ...
    return RSChatMessage(user: self.user, image: UIImage.localImage("camera-icon", template: true))
  }
...
extension RSChatThreadViewController: MessagesLayoutDelegate {
...
extension RSChatThreadViewController {
  func inputBar(_ inputBar: InputBarAccessoryView, didPressSendButtonWith text: String) {
    let message = RSChatMessage(messageId: UUID().uuidString,
...
extension RSChatThreadViewController: MessagesDisplayDelegate {
  ...
  func configureAvatarView(_ avatarView: AvatarView, for message: MessageType, at indexPath: IndexPath, in messagesCollectionView: MessagesCollectionView) {
    if let message = message as? RSChatMessage {
...
extension RSChatThreadViewController: UIImagePickerControllerDelegate, UINavigationControllerDelegate {
